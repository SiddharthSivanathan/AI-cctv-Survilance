...
Creates the Phase 3 schema: organizations, users, memberships, refresh tokens,
email verification / password reset tokens, and audit logs. Enables Row-Level
Security on audit_logs (the first tenant-scoped table) as the isolation
pattern that stores/cameras/events will follow.

Revision ID: 0001_auth_and_tenancy
Revises:
...
revision: str = "0001_auth_and_tenancy"
...
def upgrade() -> None:
    ...
    # Row-Level Security: reads/updates/deletes are scoped to the active tenant;
    # inserts are allowed (audit writes are trusted server-side). FORCE applies
    # RLS even to the table owner (the app role).
